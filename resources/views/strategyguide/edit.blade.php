...
@section("pageTitle", "Edit Strategy Guide")
...
<div class="gameLandingContainer fullscreen positionRelative" style="background-image: url('{{ $strategyGuide->game->cover[array_rand($strategyGuide->game->cover)] }}');">
    <div class="content hideOnMobile centerHorizontalVertical" style="width: 75%;">
        <div class="editor-container" style="background: #454545; padding: 10px; border-radius: 10px;">
            <h1 class="fontAlfaSlabOne colorOrange">{{ $strategyGuide->game->name }}</h1>
            <table>
                <tbody>
                    <tr>
            ...
            <div class="editor" id="title" style="background: #f9f9f9; min-height: 20px; color: black; outline: none;" contenteditable>{!! $strategyGuide->title !!}</div>
            <br>

            <div class="editor" id="editor" style="background: #f9f9f9; min-height: 200px; max-height: 200px; color: black; outline: none; overflow-y: auto;" contenteditable>{!! $strategyGuide->content !!}</div>
...
<script>
    function updateStrategyGuide() {
        ...
        if (confirm("Are you sure you want to post this? Make sure that everything is how you want it before posting.\n\n*MAKE SURE IT ADHERES TO OUR RULES.")) {
            ...
            $.ajax({
                    url: "{{ route("ajax.strategyguide.edit") }}",
                    type: "POST",
                    data: {
                    ...
                    success: (response) => {
                        if (response.success) {
                            alert("Strategy Guide was successfully created");
                            window.location.href = "{{ route("strategyguide.view", $strategyGuide->id) }}";
                        }
                    },
                    error: (response) => {
