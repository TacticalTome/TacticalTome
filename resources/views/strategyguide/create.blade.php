...
@section("pageTitle", "Create Strategy Guide")
...
<div class="gameLandingContainer fullscreen positionRelative" style="background-image: url('{{ $game->cover[array_rand($game->cover)] }}');">
    <div class="content hideOnMobile centerHorizontalVertical" style="width: 75%;">
        <div class="editor-container" style="background: #454545; padding: 10px; border-radius: 10px;">
            <h1 class="fontAlfaSlabOne colorOrange">{{ $game->name }}</h1>
            <table>
                <tbody>
                    <tr>
...
<script>
    function postStrategyGuide() {
        ...
        if (confirm("Are you sure you want to post this? Make sure that everything is how you want it before posting.\n\n*MAKE SURE IT ADHERES TO OUR RULES.")) {
            ...
            $.ajax({
                    url: "{{ route("ajax.strategyguide.create") }}",
                    type: "POST",
                    data: {
                        game_id: {{ $game->id }},
                        title: title, 
                    ...
                    success: (response) => {
                        if (response.success) {
                            ...
                            window.location.href = response.strategyguide_url;
                        }
                    },
                    error: (response) => {
    ...
    function backToGamePage() {
        if (confirm("Are you sure you want to leave?\n\nAll progress will be lost!")) {
            window.location.href = "{{ route("game.view", $game->id) }}";
        }
    }
</script>
@endpush